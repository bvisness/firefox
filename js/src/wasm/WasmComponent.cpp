...
bool Component::saveExportNameForAlias(ComponentSortIndex sortIndexOfAlias,
  ...
  if (!aliasNames_.add(p, sortIndexOfAlias, std::move(name))) {
    return false;
  }
  return true;
}
...
[[nodiscard]] static bool GetNamedPropertyFromObject(
...
[[nodiscard]] static bool GenerateDummyLoweredFunc(
  ...
  CompilerEnvironment compilerEnv(CompileMode::Once, Tier::Baseline,
                                  DebugEnabled::False);
...
bool ComponentInstance::init(JSContext* cx) {
  ...
  for (uint32_t i = 0; i < component_->coreInstances().length(); i++) {
    ...
    const auto& desc = instanceDesc.desc().as<CoreInstanceDescFromModule>();
    ...
    for (const Import& imp : mod->moduleMeta().imports) {
      ...
      WasmInstanceObject* sourceInstance = coreInstances_[instanceIndex];
      if (sourceInstance) {
        ...
        if (!GetNamedPropertyFromObject(cx, exportsObj, imp.field,
                                        &exportVal)) {
          return false;
        }
      } else {
        ...
        switch (actualItem.kind()) {
          ...
          case ComponentItem::ItemKind::Alias: {
            ...
            if (!GetNamedPropertyFromObject(cx, exportsObj, exportName,
                                            &exportVal)) {
              return false;
            }
          } break;
